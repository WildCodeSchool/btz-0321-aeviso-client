...
function Modal({ title, buttons, children }: Iprops): JSX.Element {
  ...
  return (
    <Dialog open={isOpen} onClose={handleClose} className="flex justify-center items-center fixed inset-0">
      <Dialog.Overlay className="fixed inset-0 z-50 bg-black bg-opacity-80" />

      <div className="p-10 w-10/12 sm:w-96 shadow-mainShadow flex flex-col justify-start z-50 text-white bg-blue rounded-md break-all">
          ...
          {buttons.map((button, index) => (
            <button
              ...
              className={`focus:outline-none h-8 px-6 ${index === 0 ? 'mr-2' : 'mx-2'} ${
                button.backgroundColor || 'bg-green shadow-buttonShadow'
              } rounded-sm`}
